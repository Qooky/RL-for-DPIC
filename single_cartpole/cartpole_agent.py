...
class CartPoleEnv(gym.Env):
    ...
    def __init__(self, render_mode="", n_observations=1000, q_table=np.array(None)):
    ...
    def bucket_observation(self, observation):
       return (observation < 0).astype(int)
    
    def bucket_observation_two(self, observation):
       return

    def update_q_table(self, observation, action, reward):
      self.q_table[action][self.get_index(observation)] += reward
    ...
    def train(self):
        ...
        for i in range(self.n_episodes):
          ...
          self.update_q_table(observation, action, reward)
          j +=1
